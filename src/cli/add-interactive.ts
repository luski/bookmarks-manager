...
async function promptInput(prompt: string, defaultValue: string = '', options: string[] = []): Promise<string> {
  try {
...
async function addBookmarkInteractive() {
  try {
    ...
    const url = await promptInput('🔗 Enter URL:', defaultUrl, defaultUrl ? [defaultUrl] : []);
    ...
    // Validate URL
    if (!url.match(/^https?:\/\//)) {
      execSync(`notify-send 'Invalid URL' 'URL must start with http:// or https://'`);
      process.exit(1);
    }
    ...
    execSync(`notify-send 'Add Bookmark - Step 2/4' 'Enter a title for: ${url.substring(0, 40)}...'`);
    const title = await promptInput('📝 Enter Title:', defaultTitle, [defaultTitle]);
    ...
    execSync(`notify-send 'Add Bookmark - Step 3/4' 'Enter description (or leave empty)'`);
    const descOptions = [
      '💬 Description (optional - press Esc to skip)',
      ''
    ];
    const description = await promptInput('💬 Description (optional):', '', descOptions);

    // Step 4: Prompt for tags (optional)
    execSync(`notify-send 'Add Bookmark - Step 4/4' 'Enter tags (or leave empty)'`);
    const tagsOptions = [
    ...
    // Show progress
    execSync(`notify-send 'Processing...' 'Downloading favicon and saving bookmark'`);
    ...
    const bookmark = BookmarkModel.create({
      title,
      url,
      description: description && !description.includes('optional') ? description : undefined,
      tags: tags && !tags.includes('optional') ? tags : undefined,
      favicon: faviconPath
    });

    execSync(`notify-send '✅ Bookmark Added!' '${title}' -i ${faviconPath || 'bookmark'}`);
    console.log('Bookmark added:', bookmark);
  } catch (error: any) {
    if (error.message?.includes('UNIQUE constraint failed')) {
      execSync(`notify-send 'Error' 'Bookmark already exists'`);
    } else {
      execSync(`notify-send 'Error' 'Failed to add bookmark: ${error.message}'`);
      console.error(error);
    }
    process.exit(1);
  }
